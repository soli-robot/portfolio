...
# ros2 topic pub --once /robot2/catch_thief_2 std_msgs/msg/Bool "{data: true}" 8번 좌표 계산 중단 코드
# ros2 topic pub --once /robot2/catch_thief_2 std_msgs/msg/Bool "{data: false}" 8번 좌표 계산 재시작


class IntegratedYOLONode(Node):
    def __init__(self, robot_ns='/robot8'):
        super().__init__(
        ...
        self.catch_thief_2 = False
        self.catch_thief_8_sent = False
        ...
        self.catch_thief_8_pub = self.create_publisher(Bool, 'catch_thief_8', 10)
        ...
        self.catch_thief_2_sub = self.create_subscription(
            Bool,
            '/robot2/catch_thief_2',
            self.catch_thief_2_callback,
            10
        )
    ...
    def catch_thief_2_callback(self, msg: Bool):
        self.catch_thief_2 = msg.data

        if self.catch_thief_2:
            self.safe_warn("⛔ robot2가 먼저 catch_thief_2=True → 우리 쪽 도둑 좌표 계산 중단")
        else:
            self.safe_info("✅ catch_thief_2=False → 우리 쪽 도둑 좌표 계산 가능")
            self.catch_thief_8_sent = False
        ...
        try:
            ...
            if self.trigger:
                ...
                if best_thief is not None:
                    ...
                    if self.catch_thief_2:
                        self.safe_warn("⛔ catch_thief_2=True 상태라 도둑 좌표 계산 생략")
                    else:
                        ...
                        if pt_map is not None:
                            if not self.catch_thief_8_sent:
                                self.safe_publish(self.catch_thief_8_pub, Bool(data=True))
                                self.catch_thief_8_sent = True
                                self.safe_info("✅ catch_thief_8=True 최초 발행")
            ...
            self.enqueue_frame(vis, f"{self.robot_name}_cam1")
...
def main():
    ...
    node = IntegratedYOLONode(robot_ns='/robot8')
